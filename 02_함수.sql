...
-- 함수는 SELECT 문의
-- SELECT절, WHERE절, ORDER BY 절, GROUP BY절, HAVING절 사용 가능
...
-- 문자열을 5번째 문자부터 검색하여 첫 번째 B의 위치 조회
SELECT INSTR('AABAACAABBAA','B', 5)  --9 
FROM DUAL;

-- 문자열을 5번째 문자부터 검색하여 두 번째 B의 위치 조회
...
-- 컬럼이나 문자열에서 지정한 위치부터 지정된 길이만큼 문자열을 잘라내거 반환
-- 잘라낼 길이 생략 시 끝까지 잘라냄
...
--123.5, 소수점 두 번째 자리에서 반올림(소수점 첫 번째 자리까지 표기)
...
SELECT TRUNC(123.456, 1) FROM DUAL; --123.4, 소수점 첫 째자리 아래를 절삭
SELECT TRUNC(123.456, -1) FROM DUAL; --120, 10의 자리 아래를 절삭

---------------------------------------------------------

-- 날짜(DATE) 관련 함수

-- SYSDATE : 시스템에 현재시간(년, 월, 일, 시분초)을 반환
SELECT SYSDATE FROM DUAL; --2025-03-10 12:11:16.000

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL; --2025-03-10 12:12:13.055 +0900
...
SELECT TO_CHAR(1234) FROM DUAL; --'1234' VARCHAR2타입, 숫자 > 문자

SELECT TO_CHAR(1000000,'9,999,999') ||'원' 돈 FROM DUAL;
SELECT TO_CHAR(1000000,'L9,999,999') ||'원' 돈 FROM DUAL; -- 원화 기호

-- 날짜 -> 문자 변환 시 포맷 패턴
-- YYYY : 년도 / YY : 년도 (뒤에 2글자)
-- MM : 월
-- DD : 일
-- AM 또는 PM : 오전/오후 표기
--HH : 시간 / HH24 : 24시간 표기법
...
--TO DATE(문자형 데이터, [포맷]) : 문자형 -> 날짜로 변경
--TO DATE(숫자형 데이터, [포맷]) : 숫자형 -> 날짜로 변경
-- 지정된 포맷으로 날짜를 인식함
...
-- 1)주민번호에서 - 앞글자까지 추출
SELECT EMP_NAME 이름, SUBSTR(EMP_NO, 1, INSTR(EMP_NO,'-') -1) 생년월일
FROM EMPLOYEE;

-- 2) 추출한 생년월일을 TO_DATE 타입으로 변경 -> R 패턴을 이용하여 1900년도로 변경
SELECT EMP_NAME 이름, 
TO_DATE(SUBSTR(EMP_NO, 1, INSTR(EMP_NO,'-') -1),'RRMMDD') 생년월일
FROM EMPLOYEE;
-- 1962-12-31 00:00:00.000

--3) TO_CHAR를 이용해서 문자열로 변화 -> 1962년 12월 31일
SELECT EMP_NAME 이름, 
...
SELECT EMP_NAME, JOB_CODE, SALARY,
       ...
       DECODE(JOB_CODE, 'J7', SALARY * 1.2, 'J6', SALARY * 1.15,'J5', SALARY * 1.1, SALARY *1.05) AS 인상된_급여
FROM EMPLOYEE;
...
SELECT SUM(SALARY), ROUND(AVG(SALARY)) -- 3번째 해석 
FROM EMPLOYEE						--1번째 해석							
WHERE DEPT_CODE = 'D9';  --2번째 해석
--SUM) 17700000  AVG) 5900000
...
-- 급여 최소값, 가장 빠른 입사일, 알파벳순서가 가장빠른 이메일 조회

...
/* VIEW 를 이용해서 DML 사용하기 + 문제점 확인 */

-- DEPARTMENT 테이블을 복사한 DEPT_CODE2 생성(테이블로 생성)
...
--> 모든 컬럼 값이 INSERT가 된 것이 아니라
-- VIEW를 생성할 때 사용된 컬럼에만 데이터가 삽입되었고
-- 사용되지 않는 컬럼(DEPT_TITLE)에는 NULL이 들어감
--> NULL 은 DB의 무결성을 약하게 만드는 주요 원인 
-- 가능하면 의도되지 않는 NULL은 존재하지 않게 해야한다.
...
-- 왜 사용할까?
--> VIEW 이용해서 DML(INSERT/UPDATE/DELETE) 막기 위해.
...
/* 시퀀스 삭제하기 */ 
DROP SEQUENCE SEQ_TEST_NO; -- DROP SEQUENCE 시퀀스명
...
-- NEXTVAL를 호출할 때마다 
-- INCREMENT BY 에 작성된 수만큼 증가하는 지 확인
...
-- TB_TEST 테이블에 PK값을 SEQ_TEST_NO 시퀀스로 생성
INSERT INTO TB_TEST VALUES (SEQ_TEST_NO.NEXTVAL, '짱구'); --120
INSERT INTO TB_TEST VALUES (SEQ_TEST_NO.NEXTVAL, '철수'); --125
INSERT INTO TB_TEST VALUES (SEQ_TEST_NO.NEXTVAL, '유리'); --130
...
-- EMPLPYEE 테이블의 EMP_NAME 컬럼에 인덱스 생성
...
-- * PL/SQL 이란? 
-- 오라클 데이터베이스에서 사용하는 정차적 확장 언어.
-- 변수, 조건문(IF/CASE), 반복문(LOOP, WHITE) 사용 가능
BEGIN 
	FOR I IN 1..1000000 	-- I라는 변수는 1부터 100만까지 반복한다
	LOOP
...
/* 인덱스 X */
-- TEST_ID 가 'TEST50000' 인 행 조회하기
SELECT * FROM TB_IDX_TEST
WHERE TEST_ID = 'TEST500000'; --0.012s

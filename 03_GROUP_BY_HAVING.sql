 ...
 * 6. ORDER BY 컬럼명 | 별칭 | 컬럼순번 정렬방식(ASC/DESC) [NULLS FIRST | NULLS LAST];
 * 
 * 
 * */
...
-- 그룹으로 묶인 값에 대해서 SELECT 절에서 그룹함수를 사용함

-- 그룹함수는 단 한개의 결과값을 산출하기에 그룹이 여러개일 경우 오류발생
...
--2) 전체 급여 합 조회
SELECT SUM(SALARY) FROM EMPLOYEE; --1행 70096240
...
SUM(SALARY) "급여 합"
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용X (SELECT절 해석 X)
/*4*/ORDER BY "인원 수"; -- 별칭사용 O(SELECT 절 해석 완료)
...
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5','D6') -- 부서코드가 D5, D6로 거름
GROUP BY DEPT_CODE -- WHERE 절에서 구한 부서코드가 D5, D6인 데이들을 그룹으로 묶음
ORDER BY DEPT_CODE; -- 오름차순으로 수정
...
-- 그룹함수가 적용되지 않은 컬럼을
-- 모두 GROUP BY 절에 작성되어있어야 함.

-- EMPLOYEE 테이블에서 (FROM)
-- 부서별로 같은 직급인 사원의 인원수를 조회 (GROUP BY절 DEPT_CODE, JOB_CODE 묶음)
-- 부서코드 오름차순, 직급코드 내림차순으로 정렬 (ORDER BY절)
-- 부서코드, 직급코드, 인원수 (SELECT 절)

SELECT DEPT_CODE, JOB_CODE, COUNT(*) 
FROM EMPLOYEE 
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE로 그룹을 나누고,
																-- ㄴ나눠진 그룹 내에서 JOB_CODE로 그룹을 분류
...
-- EMPLOYEE 테이블에서
-- 부서별 평균 급여가 300만원 이상인 부서의
-- 부서코드, 평균급여 조회
-- 부서코드 오름차순

SELECT DEPT_CODE 부서, ROUND(AVG(SALARY)) "평균 급여"
FROM EMPLOYEE
-- WHER SALARY >= 3000000 --> 한 사람의 급여가 400만원 이상이라는 조건(요구사항 X)
GROUP BY DEPT_CODE
 ...
 * 
 * - UNION (합집합) : 두 SELECT 결과를 하나로 합침. 단 중복은 한 번만 조회
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, 200
FROM EMPLOYEE
WHERE SALARY > 3000000;

 ...
 * 		UPDATE			 TABLE, VIEW
 * 		DELETE			 TABLE, VIEW
 * 		ALTER	    	 TABLE, SEQUENCE (수정권한 - 구조변경)	  
...
-- CREATE USER 사용자명 IDENTIFIED BY 비밀번호;
 CREATE USER ara_sample IDENTIFIED BY 1234;
...
GRANT CREATE SESSION TO ara_sample;
...
GRANT CREATE TABLE TO ara_sample; -- 테이블 생성 권한 부여

ALTER USER ara_sample DEFAULT TABLESPACE  -- TABLESPACE 할당
SYSTEM QUOTA UNLIMITED ON SYSTEM;  -- UNLIMITED 여기가 용량 100M 바꿀 수도 있음
...
-- RESOURCE : DB 시용을 위한 기본 객체 생성 권한을 묶어둔 ROLE
--> CREATE TABLE, CREATE SEQUENCE,..

-- (sys) sample 계정에 CONNECT, RESOURCE ROLE 부여

GRANT CONNECT, RESOURCE TO ara_sample;
...
GRANT SELECT ON EMPLOYEE TO ara_sample;

-- 3. (sample) 다시 kh 계정의 EMPLOYEE 조회
SELECT * FROM kh.EMPLOYEE; -- 조회 O
SELECT * FROM kh.DEPARTMENT; -- 해당 테이블은 권한을 안 줘서 조회X
--> 위에서 kh가 DEPARTMENT 권한은 주지 않아서
...
REVOKE SELECT ON EMPLOYEE FROM ara_sample;

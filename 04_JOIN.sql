...
--DEPARTMENT테이블, LOCATION 테이블 참조하여
-- 부서명, 지역명 조회
...
ON(DEPT_CODE = DEPT_ID); --23행(DEPT_CPDE가 NULL인 하동운, 이오리 포함)
...
-- 마케팅부, 국내영업부, 해외영업 3부와 매칭되는 사원이 EMPLOYEE 테이블에 없음
...
--NATURAL JOIN JOB;
--> 위 아래 둘다 결과 값이 같다 밑에는 이너조인 ANSI버전
...
-- ORACLE 버전
SELECT EMP_ID, EMP_NAME, JOB.JOB_NAME, LOCAL_NAME, SALARY
...
SELECT EMP_NAME,JOB.JOB_NAME, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE, JOB, DEPARTMENT, LOCATION
WHERE (EMPLOYEE.JOB_CODE = JOB.JOB_CODE)
AND (EMPLOYEE.DEPT_CODE = DEPARTMENT.DEPT_ID )
AND (DEPARTMENT.LOCATION_ID = LOCATION.LOCAL_CODE)
...
SELECT EMP_NAME,JOB_NAME, SALARY, SALARY * 12 * NVL(BONUS,1) "연봉"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
-- 조회하시오.(SELF JOIN 사용)
SELECT E1.EMP_NAME 사원명, E1.DEPT_CODE 부서코드, E2.EMP_NAME "동료 이름"
FROM EMPLOYEE E1
LEFT JOIN EMPLOYEE E2 ON(E1.EMP_NAME <> E2.EMP_NAME);
